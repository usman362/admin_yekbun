                        ...
                        <div class="card-body">
                            <div class="post-image">
                                <a data-fancybox="post1">
                                    <img src="{{ asset('storage/'.$card->image) }}" style="width: 375px;height:264px" alt="">
                                </a>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="deleteReasonModalLabel"><i class="bi bi-exclamation-triangle-fill text-warning"></i> Denied Request</h5>
            ...
            <div class="modal-body">
                ...
                <form id="deleteReasonForm" action="" method="POST">
                    ...
                    @foreach($reason as $key => $items)
                        <div class="form-check mb-3 d-flex justify-content-between align-items-center custom-form-check">
                            <div>
                                <label class="form-check-label d-flex align-items-start" for="reason{{ $key }}" data-title="{{ $items->title }}" data-description="{{ $items->reason }}">
                            ...
                            <input class="form-check-input" type="radio" name="reason" id="reason{{ $key }}" value="{{ $items->id }}" 
    ...
    function confirmAction(event, cardId) {
        ...
        document.querySelectorAll('input[name="reason"]').forEach(radio => {
            radio.addEventListener('change', function() {
                ...
                document.getElementById('reasonDescriptionInput').value = this.getAttribute('data-description');
            });
        });
