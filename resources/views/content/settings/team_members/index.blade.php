                        ...
                        <tr>
                            ...
                            <td>
                                @foreach ($user->roles as $role)
                                    <span
                                        class="badge {{ $role->name === 'Super Admin' ? 'bg-label-primary' : 'bg-label-dark' }}">
                                        {{ $role->name }}
                                    </span>
                                @endforeach
                            </td>
                            ...
                            <td>
                                ...
                                @if ($user->roles->isNotEmpty())
                                    <button class="btn btn-sm btn-icon" data-bs-toggle="modal"
                                        ...
                                        <img src="{{ asset('assets/img/Update.svg') }}" alt="Edit Roles"
                                ...
                                @if (!$user->is_superadmin)
                                    <form action="{{ route('settings.team.members.destroy', $user->id) }}" method="POST"
                                        class="d-inline" onsubmit="confirmAction(event, () => event.target.submit())">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-icon">
                                            <img src="{{ asset('assets/img/Remove.svg') }}" alt="Delete"
                                                style="width: 18px; height: 18px;">
                                        </button>
                                    </form>
                                @endif
