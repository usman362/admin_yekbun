...
<div class="card">
  <div class="card-header d-flex align-items-center justify-content-between">
    <h5 class="m-0" style="font-size: 24px;"> <strong>Paypal List</strong></h5>
  ...
  <div class="table-responsive text-nowrap  mb-4">
    <table class="table">
      ...
      <tbody class="table-border-bottom-0">
        @forelse($paypal as $bank)
        <tr>
          <td>{{ $loop->iteration }}</td>
          <td>{{ $bank->add_title ?? '' }}</td>
          <td>{{ $bank->paypal_cleint_id ?? '' }}</td>
          <td>{{ $bank->paypal_cleint_secret ?? '' }}</td>

          <td>
            <div class="dropdown d-inline-block">
              <!-- Edit -->
              <span data-bs-toggle="modal" data-bs-toggle="modal" data-bs-target="#editpaypalModal{{ $bank->id }}">
              ...
              <!-- Delete -->
              <form action="{{ route('settings.paypal.destroy',$bank->id) }}" onsubmit="confirmAction(event, () => event.target.submit())" method="post" class="d-inline">
                @method('DELETE')
                @csrf
            ...
            <x-modal id="editpaypalModal{{ $bank->id }}"
              title="Edit Paypal "
              saveBtnText="Update"
              saveBtnType="submit"
              saveBtnForm="editForm{{ $bank->id }}"
              size="md">
              @include('content.paypaltransfer.editForm')
            </x-modal>
          </td>
        </tr>
        @empty
...
<!--/ Basic Bootstrap Table -->
<x-modal id="createpaypalModal" title="Create Paypal" saveBtnText="Create" saveBtnType="submit" saveBtnForm="createForm" size="md">
  @include('content.include.paypal-transfer.createForm')
</x-modal>
