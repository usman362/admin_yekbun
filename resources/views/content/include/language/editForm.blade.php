    ...
    <div class="modal-dialog modal-md modal-simple modal-dialog-centered modal-add-new-role">
        <div class="modal-content p-0">
            ...
            <div class="modal-body">
                ...
                <form id=createForm method="post" action="{{ route('language.update', $language->id) }}"
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select class="form-select" aria-label="Default select example" name="status">
                                        <option value="" {{ is_null($language->status) ? 'selected' : '' }}>Select
                                        </option>
                                        <option value="0" {{ $language->status == 0 ? 'selected' : '' }}>UnPublish
                                        </option>
                                        <option value="1" {{ $language->status == 1 ? 'selected' : '' }}>Publish
                                        </option>
                                    </select>
                                    @error('status')
                                        <span class="text-danger">{{ $message }}</span>
