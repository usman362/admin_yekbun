...
@foreach ($userTypes as $type)
    <p>{{ ucfirst($type) }}: {{ $femaleCounts[$type] ?? 0 }}</p>
@endforeach
...
@endforeach --}}
    ...
    <div class="bg-trophy-loader">
        <div class="d-flex justify-content-between">
            @foreach ($userTypes as $key => $type)
                <div class="trophy-card">
                    <div class="spinner-border {{ $key == 1 ? 'text-warning' : ($key == 2 ? 'text-info' : 'text-success') }}"
                        style="width: 110px; height: 110px;" role="status">
                        <span class="visually-hidden">Loading...</span>
                    </div>
                    <div class="d-flex justify-content-center align-items-center">
                        <img src="{{ asset('assets/svg/svg-dialog/' . $type . '.svg') }}"
        ...
        <div class="mini-stat-box mt-1">
            <div class="d-flex">
                <img src="{{ asset('assets/svg/svg-dialog/male.svg') }}" width="35" style="height: 30px">
                @foreach ($userTypes as $key => $type)
                    @if ($key === 0)
                        <div class="detail-progress mx-2">
                            <span><img src="{{ asset('assets/svg/svg-dialog/' . $type . '.svg') }}" width="18"
                                    alt>&nbsp;
                                {{ $femaleCounts[$type] ?? 0 }}</span>
                            <div class="progress mt-1"
                                style="width: 60px; height: 8px; background: {{ $key == 1 ? '#F9E59E80' : ($key == 2 ? '#1CA2ED4D' : '#1BC4694D') }};">
                                <div class="progress-bar {{ $key == 1 ? 'bg-warning-01' : ($key == 2 ? 'bg-info-01' : 'bg-success-01') }}"
        ...
        <div class="mini-stat-box mt-1">
            <div class="d-flex">
                <img src="{{ asset('assets/svg/svg-dialog/female.svg') }}" width="35" style="height: 30px">
                @foreach ($userTypes as $key => $type)
                    @if ($key === 0)
                        <div class="detail-progress mx-2">
                            <span><img src="{{ asset('assets/svg/svg-dialog/' . $type . '.svg') }}" width="18"
                                    alt>&nbsp;
                                {{ $maleCounts[$type] ?? 0 }}</span>
                            <div class="progress mt-1"
                                style="width: 60px; height: 8px; background: {{ $key == 1 ? '#F9E59E80' : ($key == 2 ? '#1CA2ED4D' : '#1BC4694D') }};">
                                <div class="progress-bar {{ $key == 1 ? 'bg-warning-01' : ($key == 2 ? 'bg-info-01' : 'bg-success-01') }}"
                                    role="progressbar" style="width: 50%" aria-valuenow="25" aria-valuemin="0"
    ...
    <div class="gender-status mt-1">
        ...
        @php
            // Step 1: Total reviews from all age groups (male + female)
            $totalReviews = collect($statistics)->sum(function ($stat) {
                return $stat['male']['reviews'] + $stat['female']['reviews'];
            });
        @endphp

        @foreach ($statistics as $index => $statistic)
            @php
                $groupReviews = $statistic['male']['reviews'] + $statistic['female']['reviews'];
                $percentage = $totalReviews > 0 ? number_format(($groupReviews / $totalReviews) * 100, 1) : 0;
            @endphp
            <!-- Bar 1 -->
            <div class="d-flex align-items-center mb-2">
                <span class="me-2 text-age" style="width: 50px;">{{ $statistic['age'] }}</span>
                <div class="flex-grow-1 age-bar mb-0"
                    style="background-color: {{ $statistic['female']['reviews'] > 0 ? '#cc8ff5' : '#fff' }}">
                    <div class="male"
                        style="width: {{ ($statistic['male']['reviews'] * 100) / $statistic['max'] }}%"></div>
                    <div class="female"
                        style="width: {{ ($statistic['female']['reviews'] * 100) / $statistic['max'] }}%"></div>
                </div>
    ...
    <div class="gender-status mt-1">
        <h6 class="fw-bold text-province">List of
            Provinces</h6>
        @php
            // Define provinces and initialize vote counts
            $provincesList = ['Rojava', 'Bakûr', 'Rojhilat', 'Başûr'];
            $provinceVotes = [];
            $totalVotes = 0;
            ...
            // Fill actual data
            foreach ($province_statistics as $item) {
                if (in_array($item['province'], $provincesList)) {
                    $provinceVotes[$item['province']] = $item['total_votes'];
                }
            }

            // Total votes
            $totalVotes = array_sum($provinceVotes);
        @endphp

        @foreach ($provincesList as $index => $provinceName)
            @php
                $votes = $provinceVotes[$provinceName];
                $percent = $totalVotes > 0 ? round(($votes / $totalVotes) * 100, 1) : 0;
                if ($percent < 40) {
                    $percent = 40;
                }
            @endphp
            <div class="province-item province-{{ $index + 1 }}" style="width: {{ $percent }}%">
                <span>Kurdistan – {{ $provinceName }}</span>
                <span>{{ number_format($provinceVotes[$provinceName]) }}</span>
            </div>
        @endforeach
    ...
    <div class="p-2">
        <div class="community p-3">
            ...
            <div class="d-flex fw-bold" style="color: #333">
                ...
                <div class="flex-fluid d-flex justify-content-end align-items-center total-reviews">
                    {{ $total_reviews }}
                </div>
            </div>
            <div class="vote-reaction dislike d-flex align-items-center mt-4"
                style="width:{{ $total_dislikes > 1 ? round(($total_dislikes * 100) / $total_reviews) : '10' }}%">
            ...
            <div class="vote-reaction noIdea d-flex align-items-center mt-2"
                style="width:{{ $total_neutrals > 1 ? round(($total_neutrals * 100) / $total_reviews) : '10' }}%">
            ...
            <div class="vote-reaction like d-flex align-items-center mt-2"
                style="width:{{ $total_likes > 1 ? round(($total_likes * 100) / $total_reviews) : '10' }}%">
                    ...
                    <div class="flex-fluid d-flex justify-content-between align-items-center gap-3">
                        <div class="progress w-100" style="height:15px;">
                            <div class="progress-bar bg-primary" role="progressbar"
                                ...
                                aria-valuemax="{{ $statistic['max'] }}" data-toggle="tooltip"
