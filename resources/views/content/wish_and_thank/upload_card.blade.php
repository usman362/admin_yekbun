...
@section('content')
    ...
    <h4 class="fw-bold py-3 mb-4">
        <span class="text-muted fw-light">Prays </span> /Add Cards
    </h4>


    <ul class="nav nav-pills flex-column flex-md-row mb-3 d-flex justify-content-end align-item-end">

        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#createcategoryModal">Add Cards</button>
    </ul>
    <!--/ User Sidebar -->
                        ...
                        <div class="content-wrap">
                            ...
                            <div class="card-heading justify-content-between">
                                ...
                                <div class="user-block">
                                    <div class="user-info">
                                        <span class="d-flex"><a href="#">{{$card->name}}</a></span>
                                        <span class="time">{{$formattedDate}}</span>
                                    </div>
                                </div>
                                <div class="d-flex justify-content-end align-item-end">
                                    <form action="{{ route('list.prays-cards.delete',$card->id) }}" onsubmit="confirmAction(event, () => event.target.submit())" method="post" class="d-inline">
                                        @method('DELETE')
                                        @csrf
                                ...
                                <div class="post-image">
                                    <a data-fancybox="post1" data-lightbox-type="comments">
                                        <img src="{{ asset('storage/'.$card->image) }}" style="width: 375px;height:264px"
                                            data-demo-src="{{ asset('storage/'.$card->image) }}"
    ...
    <x-modal id="createcategoryModal" title="Add Background" saveBtnText="Add" saveBtnType="submit" saveBtnForm="createForm"
        ...
        @include('content.include.wishes.createForm2')
    </x-modal>
    <!-- /Modal -->
    <script>
