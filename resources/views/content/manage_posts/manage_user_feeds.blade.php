                                ...
                                <div class="post-image">
                                    ...
                                    <div class="nav-item dropdown d-block"
                                        ...
                                        <a class="nav-link dropdown-toggle hide-arrow" href="#"
                                            ...
                                            <div class="d-flex align-items-center gap-2 open-edit-modalfree"
...
"
    ...
    <div class="modal fade" id="open-edit-modalfree" tabindex="-1" aria-labelledby="editFeedModalLabelfeee"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" style="max-width: 98%; width: 98%;">
            <div class="modal-content"
                style="background: transparent; border: none; box-shadow: none; width:800px !important;">
                <div class="d-flex flex-row justify-content-between align-items-start"
                    ...
                    <div class="bg-white shadow"
                        style="width: 48%; position: relative; border-radius:12px !important">
                        <button type="button" class="close position-absolute" style="top: -27px; right: -56px;"
                            onclick="$('#open-edit-modalfree').modal('hide')">&times;</button>
                        <div class="reported-feeds">
                            <div class="header mb-3">
                                <h2>Reported Feeds</h2>
                                <p>Manage User Feeds</p>
...
<script>
    ...
    // Event listener for 'open-edit-modal' - This will open the 'editFeedModal'
    document.querySelectorAll('.open-edit-modal').forEach(function(button) {
        button.addEventListener('click', function() {
            ...
            // Load content based on the section for the 'editFeedModal'
            ...
            // Insert the content into the modal for 'editFeedModal'
    ...
    // Event listener for 'open-edit-modalfree' - This will open the 'open-edit-modalfree' modal
    document.querySelectorAll('.open-edit-modalfree').forEach(function(button) {
        button.addEventListener('click', function() {
            const feedId = this.dataset.id;
            const commentId = this.dataset.commentId;
            const section = this.dataset.section;
            ...
            // Load content based on the section for the 'open-edit-modalfree' modal
            if (section === 'reported-feeds') {
                const feedElement = document.querySelector(`#feed-post-${feedId}`);
                contentHtml = feedElement ? feedElement.cloneNode(true).outerHTML : 'Feed not found.';
            } else if (section === 'reported-comments') {
            ...
            // Insert the content into the modal for 'open-edit-modalfree'
            const modalBody = document.getElementById('editFeedContentfree');
            modalBody.innerHTML = contentHtml;

            // Show the modal
            const modal = new bootstrap.Modal(document.getElementById('open-edit-modalfree'));
            modal.show();
        });
    });

    // Clear modal content when it's closed for 'editFeedModal'
    document.getElementById('editFeedModal').addEventListener('hidden.bs.modal', function() {
        const modalBody = document.getElementById('editFeedContent');
        modalBody.innerHTML = ''; // Clear modal content
    });

    // Clear modal content when it's closed for 'open-edit-modalfree'
    document.getElementById('open-edit-modalfree').addEventListener('hidden.bs.modal', function() {
        const modalBody = document.getElementById('editFeedContentfree');
        modalBody.innerHTML = ''; // Clear modal content
    });
});
